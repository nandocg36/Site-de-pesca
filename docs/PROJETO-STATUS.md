...
**Última atualização do documento:** 2026-03-29 (Marco 3b: remoto Supabase aplicado + PR `marco3b-dues`)
...
| Dimensão | Situação |
|----------|----------|
| **Código no repositório** | **Marcos 1–3 + Marco 3b (slice 1):** `apps/web` com `/`, `/instalar-pwa`, `/convite`, `/feed`, **`/mensalidade`**, **`/admin`**, **`/admin/presenca`**, **`/admin/mensalidade`**, **`/carteirinha`**; `packages/fishing-core`; `supabase/migrations` inclui **`20260329300000_marco3b_dues.sql`**. **Backend:** Supabase **`pesca-plataforma-norte`** (`imyyiugqqioxmaxedkgn`); **Marco 3b aplicado no remoto** (schema + RPCs + RLS em `membership_dues_payments`; org demo com `dues`). **PWA legada** na raiz; `npm run dev` → app nova. **ND Fleet** pausado — ver **⏳ Pendências**. |
| **Planeamento** | **Marco 3b em curso:** slice 1 = plano titular (valor + dia), pagamento **manual**, estado mês (fuso **America/Sao_Paulo**), duplo consentimento dependente, flag org **`dues`**. **A seguir dentro do 3b:** ranking “maior peixe”, logs/auditoria fina, **PSP** (cartão/PIX/link), solicitar link + WhatsApp empresa. **`apps/admin` dedicado** ou **Auth forte** continuam como opções paralelas. |
...
| `apps/web/` | React+Vite+TS: `/feed`, **`/mensalidade`**, **`/admin`**, **`/admin/presenca`**, **`/admin/mensalidade`**, **`/carteirinha`**, convite/PWA; `vitest.config.ts`; `npm run test -w @pesca/web`; `lib/memberCheckinCode.ts`, `lib/formatBrl.ts` |
...
| `supabase/migrations/` | Marco 1–3 como antes. **Marco 3b:** `profiles.holder_profile_id`, colunas `dues_*`, `membership_dues_payments`, RPCs `marco3b_*` (status sócio, partilha titular/dependente, staff plano/pagamento/lista). Flag org `feature_flags.dues` |
| `supabase/seed.sql` | Dev: org com `presence_qr` e **`dues`**; sócio demo com plano R$ 150 / dia 10 + pagamento mês corrente; **`marco1-dev-token`**; colaborador **`marco3-collab-dev-token`**; Marco 2 social demo |
...
- **Marco 3b mensalidade — slice 1 (2026-03-29):** flag org **`dues`** em `feature_flags`. **Titular** = `profiles.holder_profile_id is null`; **dependente** aponta para titular. Plano: `dues_monthly_amount_cents`, `dues_due_day` (1–28). **Duplo consentimento:** `dues_share_with_dependents` (titular) + `dues_dependent_can_see` (dependente). Tabela **`membership_dues_payments`** (manual; único por `billed_profile_id` + `covers_month`); vencimento e “em dia / atraso / a vencer” calculados no **mês corrente** com data de referência em **America/Sao_Paulo**. RPCs: `marco3b_member_dues_status`, `marco3b_member_holder_share_dues`, `marco3b_member_dependent_accept_dues`, `marco3b_staff_set_member_dues`, `marco3b_staff_record_dues_payment`, `marco3b_staff_dues_members_list`. UI: **`/mensalidade`**, **`/admin/mensalidade`**. **Fora deste slice:** PSP, links automáticos, ranking peixe, multimeses de dívida acumulada na UI, owner-only para toggles globais de módulo (hoje só flag na org).
...
|------|-------------|-----------------|----------|
| 2026-03-29 | **Deploy remoto Marco 3b** | Projeto **`imyyiugqqioxmaxedkgn`**: DDL + funções via MCP (`apply_migration` parcial inicial + `execute_sql` em blocos para RPCs); validação colunas, RLS, `has_function_privilege` para `anon`, `timezone('America/Sao_Paulo')`; `UPDATE organizations` com `dues`; seed remoto (plano sócio demo + pagamento mês). Branch **`marco3b-dues`** + PR para `main`. | Remoto Supabase; `docs/PROJETO-STATUS.md` |
| 2026-03-29 | Iniciar **Marco 3b** | **Migração** `20260329300000_marco3b_dues.sql`: colunas `dues_*`, `holder_profile_id`, `membership_dues_payments`, RPCs `marco3b_*`. **Seed:** `dues` na org demo, plano sócio demo, pagamento mês corrente. **App:** `MemberDuesPage`, `AdminDuesPage`, rotas, links feed/admin; `formatBrl` + testes Vitest. **E2E:** smoke `/mensalidade`. **Doc:** README fluxos Supabase; PROJETO-STATUS. **Skill:** `security-review` (entrada monetária validada no servidor). | `supabase/migrations/20260329300000_marco3b_dues.sql`, `supabase/seed.sql`, `apps/web/src/App.tsx`, `apps/web/src/pages/MemberDuesPage.tsx`, `apps/web/src/pages/admin/AdminDuesPage.tsx`, `AdminLayout.tsx`, `AdminDashboard.tsx`, `FeedPage.tsx`, `apps/web/src/lib/formatBrl.ts`, `formatBrl.test.ts`, `apps/web/src/index.css`, `e2e/web-smoke.spec.ts`, `README.md`, `docs/PROJETO-STATUS.md` |
...
- **Marco 3b — feito no repo + remoto (slice 1):** mensalidade manual + UI sócio/staff + migração aplicada em **`pesca-plataforma-norte`**. **Pendente no 3b:** ranking “maior peixe”; batidas/logs extra; **PSP** (BR a decidir); solicitar link + WhatsApp; multimeses de dívida na UI; RPC **owner-only** para ligar/desligar módulo `dues` (hoje `UPDATE organizations` / dashboard).
